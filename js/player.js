...
export default class Player {
    ...
    update() {
        ...
        if (velocity.x > 7) sprite.setVelocityX(2);
        else if (velocity.x < -7) sprite.setVelocityX(-2);
    ...
    /**
     * Give player item from mystery box
     * @param _itemData {number}
     */
    giveMysteryBoxItem(_itemData) {
        if (config.debug.playerDebug.state) {
            console.log(`Player picked mystery box item: [${_itemData.id}] ${_itemData.name}`)
        }

        this.mysteryBox.pickedItemData = _itemData
    }

    removeUsedMysteryBoxItem() {
        this.mysteryBox.pickedItemData = {}
    }
}
